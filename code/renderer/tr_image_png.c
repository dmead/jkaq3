...
Copyright (C) 2012      Jeremy Davis ("Ensiform")

Modified to work with newer libpng and newer ioq3 screenshot standards.
...
void R_LoadPNG (const char *name, byte **pic, int *width, int *height)
{
	...
	// load png
	size = ri.FS_ReadFile( ( char * ) name, &fbuffer.v);
	...
	png_set_read_fn(png, fbuffer.b, png_read_data);
	...
	*pic = out = (byte *) ri.Malloc(w * h * 4);
	...
	if(setjmp(png_jmpbuf(png)))
	{
		ri.Printf(PRINT_WARNING, "R_LoadPNG: second exception handler called for (%s)\n", name);
...
void RE_SavePNG(char * filename, int image_width, int image_height, byte *image_buffer, int num_bytes, int padding, qboolean flip)
{
	...
	buffer = ri.Hunk_AllocateTempMemory(image_width * image_height * num_bytes);
	...
	if(num_bytes == 4)
	{
		png_set_IHDR(png, info, image_width, image_height, 8, PNG_COLOR_TYPE_RGBA, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_DEFAULT,
				 PNG_FILTER_TYPE_DEFAULT);
	}
	else
	{
		// should be 3
		png_set_IHDR(png, info, image_width, image_height, 8, PNG_COLOR_TYPE_RGB, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_DEFAULT,
				PNG_FILTER_TYPE_DEFAULT);
	}
	...
	row_pointers = ri.Hunk_AllocateTempMemory(image_height * sizeof(png_bytep));
	...
	row_stride = image_width * num_bytes + padding;
	row = (byte *)image_buffer + (image_height - 1) * row_stride;
	...
	ri.FS_WriteFile(filename, buffer, png_compressed_size);
